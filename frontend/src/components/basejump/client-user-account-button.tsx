...
export default function ClientUserAccountButton({
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        className="w-56 border-subtle dark:border-white/10 bg-card-bg dark:bg-background-secondary rounded-xl shadow-custom"
        align="end"
        forceMount
      >
        ...
        <DropdownMenuGroup className="p-1">
          ...
          >
            <Link
              href="/dashboard"
              className="flex w-full h-full text-foreground/90"
            >
              My Account
            </Link>
          </DropdownMenuItem>
        ...
        <div className="p-1">
          <DropdownMenuItem
            asChild
            className="rounded-md hover:bg-hover-bg text-foreground/90 cursor-pointer"
          >
            <button
              onClick={handleSignOut}
              className="flex w-full h-full px-2 py-1.5 text-left"
            >
              Log out
            </button>
          </DropdownMenuItem>
